...
router.get('/', (req, res) => {
  ...
  .then(result => res.status(200).send({'name':'PM2.5MassConcentration','unit':'µg/m3','dataArray':result}))
...
router.get('/:station/:functions', async(req, res) => {
  ...
  try {
    if (functions == 'mean') {
      ...
      res.status(200).send({'name':'PM2.5MassConcentration','unit':'µg/m3','dataArray':result})
    }
    else
